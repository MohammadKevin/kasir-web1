  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_390px] h-full overflow-hidden relative">
      ...
      <div className={`flex flex-col h-full overflow-hidden space-y-4 ${mobileView === 'catalog' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        {/* Search bar */}
        <div className="flex items-center gap-3 bg-white border border-slate-200/80 px-4 py-3.5 rounded-2xl shadow-3xs flex-shrink-0 relative overflow-hidden group transition-all duration-200">
          <Search size={16} className="text-slate-400 shrink-0 group-focus-within:text-indigo-650 transition-colors" />
          <input
            ref={searchInputRef}
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Cari nama produk, SKU, atau scan barcode... (F2)"
            className="w-full text-xs outline-none bg-transparent text-slate-850 placeholder:text-slate-400 font-semibold"
          />
          <ScanLine size={16} className="text-rose-500 shrink-0 animate-pulse" />
        </div>

        {/* Category filter */}
        <div className="flex flex-wrap gap-1.5 flex-shrink-0 overflow-x-auto pb-1">
          <button
        ...
        {/* Product grid */}
        <div className="flex-1 overflow-y-auto pr-1 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 content-start pb-6 scrollbar-thin">
          {loading
            ? Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="h-56 rounded-3xl bg-slate-100 animate-pulse border border-slate-150" />
              ))
            : filtered.length === 0
            ? (
                <div className="col-span-full py-24 text-center text-slate-450 border border-dashed border-slate-200 bg-white rounded-2xl text-xs font-bold">
            ...
            : filtered.map(p => {
                ...
                return (
                  <div
                    key={p.id}
                    className={`group bg-white border rounded-2xl p-4 shadow-3xs flex flex-col justify-between relative overflow-hidden transition-all duration-300 ${
                      outOfStock 
                        ? 'opacity-60 border-slate-200' 
                        : 'border-slate-200/80 hover:border-indigo-400 hover:shadow-sm'
                    }`}
                  >
                    <div className="aspect-video w-full overflow-hidden rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 relative">
                      {p.image ? (
                        <img src={p.image} alt={p.name} className="w-full h-full object-cover transition-transform duration-350 group-hover:scale-105" />
                      ) : (
                        <Package size={22} className="text-slate-350" />
                      )}
                      
                      <span className="absolute top-2 right-2 bg-indigo-50/90 text-indigo-700 text-[8.5px] font-black px-2 py-0.5 rounded-lg border border-indigo-150 leading-none backdrop-blur-xs">
                        {getCategoryName(p.categoryId || '')}
                      </span>

                      {hasDiscount && (
                        <span className="absolute top-2 left-2 bg-gradient-to-r from-rose-500 to-rose-600 text-white text-[8.5px] font-black px-2 py-0.5 rounded-lg shadow-3xs leading-none border border-rose-400/20">
                          PROMO
                        </span>
                      )}
                      ...
                      {outOfStock && (
                        <div className="absolute inset-0 bg-white/70 flex items-center justify-center backdrop-blur-[1px]">
                          <span className="text-[10px] font-black text-rose-700 bg-white border border-rose-200 px-3 py-1 rounded-xl shadow-3xs">Habis</span>
                        </div>
                      )}
                    </div>

                    <div className="mt-3">
                      <p className="text-[8.5px] font-mono text-slate-400 uppercase tracking-widest font-semibold">{p.sku || '–'}</p>
                      <h3 className="font-extrabold text-slate-900 text-xs mt-1 line-clamp-2 leading-snug group-hover:text-indigo-650 transition-colors" title={p.name}>{p.name}</h3>
                    </div>

                    <div className="mt-4 pt-3 border-t border-slate-100 flex items-end justify-between gap-2">
                      <div>
                        <span className="text-xs font-black font-mono text-indigo-600">{fmt(discountedPrice)}</span>
                        {hasDiscount && (
                          <span className="block text-[8.5px] text-slate-400 font-mono line-through mt-0.5">{fmt(p.sellingPrice)}</span>
                        )}
                        <span className={`block text-[9px] mt-1.5 font-bold ${isLowStock ? 'text-rose-600' : 'text-slate-450'}`}>
                          Stok: {p.stock}
                        </span>
                      </div>
                      <button
                        onClick={() => addToCart(p)}
                        disabled={outOfStock}
                        className="h-8 w-8 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white flex items-center justify-center shadow-3xs transition-all active:scale-95 disabled:bg-slate-100 disabled:text-slate-400 cursor-pointer shrink-0"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
      ...
      <div className={`bg-white rounded-2xl border border-slate-200 p-5 shadow-3xs flex flex-col h-full overflow-hidden ${mobileView === 'cart' ? 'flex' : 'hidden lg:flex'}`}>
        ...
        {/* Cart items */}
        <div className="flex-1 overflow-y-auto pr-0.5 space-y-3 mb-4 flex flex-col scrollbar-thin">
                  ...
                  <div key={item.id} className="border border-slate-150 bg-slate-50/40 p-3.5 rounded-xl space-y-2.5">
                    <div className="flex justify-between items-start gap-2">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        <div className="flex items-center border border-slate-200 rounded-lg bg-white p-0.5 shadow-3xs">
                          <button onClick={() => updateQty(item.id, -1)} className="p-1 bg-white border border-slate-200 rounded-md text-slate-655 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 cursor-pointer"><Minus size={10} /></button>
                          <span className="px-2 font-extrabold text-slate-800 text-xs w-6 text-center">{item.qty}</span>
                          <button onClick={() => updateQty(item.id, 1)} className="p-1 bg-white border border-slate-200 rounded-md text-slate-655 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-250/50 cursor-pointer"><Plus size={10} /></button>
                        </div>
                        <button onClick={() => removeFromCart(item.id)} className="p-1 text-slate-350 hover:text-rose-600 cursor-pointer transition-colors"><Trash2 size={14} /></button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between gap-3 bg-white border border-slate-100 px-2.5 py-1 rounded-lg">
                      ...
                      <input
                        ...
                        className="w-24 bg-slate-50/50 border border-slate-200 rounded-lg px-2 py-0.5 text-right text-[10.5px] font-mono font-bold text-slate-850 outline-none focus:border-indigo-500"
                      />
                    </div>
                  </div>
                )
          ...
          {!isCustomerMode ? (
            ...
            <div className="space-y-2.5 bg-slate-50/50 border border-slate-200 rounded-2xl p-3.5 animate-in slide-in-from-bottom-2 duration-150">
              <div className="flex justify-between items-center">
                ...
                <button type="button" onClick={() => setIsCustomerMode(false)} className="cursor-pointer text-slate-400 hover:text-slate-655"><X size={13} /></button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                <input type="text" placeholder="Nama" value={customerName} onChange={e => setCustomerName(e.target.value)}
                  className="bg-white border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs outline-none focus:border-indigo-550 text-slate-800 font-semibold" />
              </div>
              <label className="flex items-center gap-1.5 text-[10px] text-slate-500 cursor-pointer font-semibold">
                <input type="checkbox" checked={saveCustomer} onChange={e => setSaveCustomer(e.target.checked)}
                  className="rounded border-slate-350 h-3.5 w-3.5 text-indigo-650 cursor-pointer" />
        ...
        <div className="border-t border-slate-100 pt-4 space-y-3.5 flex-shrink-0">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-center gap-1.5 py-2.5 border rounded-xl text-xs font-bold transition-all cursor-pointer ${
                      active
                        ? 'border-indigo-550 bg-indigo-50/30 text-indigo-650'
                        : 'border-slate-200 bg-white text-slate-505 hover:border-slate-350'
                    }`}
                  >
          ...
          {/* Cash input */}
          {payment === 'CASH' && (
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2 font-mono text-xs shadow-3xs">
              <div className="flex items-center justify-between">
                ...
                <div className="relative w-1/2 flex items-center justify-end">
                  <input
                    ...
                    className="bg-transparent font-bold text-slate-900 outline-none text-right w-full border-b border-slate-200 focus:border-indigo-500 pb-0.5"
                    autoFocus
                  />
                </div>
              </div>
              <div className="flex justify-between items-center border-t border-slate-100 pt-2">
                ...
                <span className={`font-bold text-xs ${change > 0 ? 'text-emerald-600' : 'text-slate-655'}`}>{fmt(change)}</span>
              </div>
            </div>
          )}
          ...
          {payment !== 'CASH' && (
            <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-150 rounded-xl px-3 py-2.5 text-[10px] text-emerald-700 font-bold shadow-3xs leading-none">
          ...
          <div className="space-y-2 pt-1">
            <button
              ...
              className="w-full rounded-xl bg-indigo-650 hover:bg-indigo-700 text-white py-3 text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-40 shadow-indigo-500/10 transition-all cursor-pointer"
            >
              {submitting
          ...
          <div className="w-full max-w-sm rounded-2xl border border-slate-150 bg-white p-6 shadow-xl space-y-4 animate-in zoom-in-95 duration-150 relative overflow-hidden">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4 font-mono text-[11px] space-y-3.5 shadow-3xs max-h-[280px] overflow-y-auto scrollbar-thin">
              <div className="text-center border-b border-dashed border-slate-300 pb-3">
                ...
                <p className="text-[8.5px] font-bold text-slate-450 mt-1">LAILA COLLECTIONS</p>
              </div>
              ...
              <div className="space-y-1.5 text-[10px] text-slate-655 border-t border-dashed border-slate-200 pt-2">
                ...
                <div className="flex justify-between font-black text-emerald-650"><span>Kembalian:</span><span>{fmt(receiptData.changeAmount)}</span></div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              ...
              <button
                ...
                className="rounded-xl bg-indigo-650 hover:bg-indigo-700 text-white py-2.5 text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-indigo-500/10 active:scale-97"
              >
                Order Baru
                <ArrowRight size={13} />
              </button>
